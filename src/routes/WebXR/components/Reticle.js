 ...
 * horizontal surface.
 */
class Reticle extends window.THREE.Object3D {
    /**
    ...
    constructor(xrSession, camera) {
      ...
      let geometry = new window.THREE.RingGeometry(0.1, 0.11, 24, 1);
      let material = new window.THREE.MeshBasicMaterial({ color: 0xffffff });
      // Orient the geometry so its position is flat on a horizontal surface
      geometry.applyMatrix(new window.THREE.Matrix4().makeRotationX(window.THREE.Math.degToRad(-90)));
  
      this.ring = new window.THREE.Mesh(geometry, material);
  
      geometry = new window.THREE.PlaneBufferGeometry(0.15, 0.15);
      // Orient the geometry so its position is flat on a horizontal surface,
      // as well as rotate the image so the anchor is facing the user
      geometry.applyMatrix(new window.THREE.Matrix4().makeRotationX(window.THREE.Math.degToRad(-90)));
      geometry.applyMatrix(new window.THREE.Matrix4().makeRotationY(window.THREE.Math.degToRad(0)));
      material = new window.THREE.MeshBasicMaterial({
        color: 0xffffff,
        transparent: true,
        opacity: 0
      });
      this.icon = new window.THREE.Mesh(geometry, material);
  
      // Load the anchor texture and apply it to our material
      // once loaded
      this.loader.load('/images/webxr/Anchor.png', texture => {
    ...
    async update(frameOfRef) {
      ...
      if (hits.length) {
        ...
        DemoUtils.lookAtOnY(this, this.camera, [0, 0]);
